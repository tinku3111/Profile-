<h1 align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=24&duration=3000&pause=900&color=7A7A7A&center=true&vCenter=true&width=500&lines=Balaji+Sangana;Offensive+Security+Researcher;Penetration+Tester" />
</h1>
<p align="center">
  <strong>Web • API • Network Security</strong>
</p>

<p align="center">
  <a href="javas\x01cript:javascript:alert(1)" id="fuzzelement1">test</a>
  <a href="https://balajisangana.pages.dev/">Website</a> •
  <a href="mailto:balajisangana731@gmail.com">Email</a> •
  <a href="https://www.linkedin.com/in/balaji-sangana">LinkedIn</a>
</p>

---

## 👋 About Me

I am an **Offensive Security Researcher & Penetration Tester** focused on identifying, exploiting, and responsibly disclosing real-world security vulnerabilities.

- 🐞 **Verified Bug Bounty Hunter** (7+ valid reports)
- ⚔️ Specialization in **Web, API & Network Attacks**
- 🔍 Strong in **Reconnaissance, Exploitation & Automation**
- 🧑‍💻 Founder & Security Consultant @ **Haakwin**

---

## 🔴 Offensive Security Skills

- Web Application Penetration Testing  
- API Security Testing (REST / Auth / Logic Flaws)  
- Network & Internal Pentesting  
- Vulnerability Research & Exploit Development  
- Reconnaissance & Attack Surface Mapping  

---

## 🧪 Skill Proficiency

| Domain | Level |
|------|------|
| Web Pentesting | ██████████ 90% |
| API Security | █████████▊ 88% |
| Network Pentesting | █████████▌ 85% |
| Vulnerability Research | █████████▋ 87% |
| Recon & Enumeration | ██████████ 92% |

---

## 💻 Programming & Scripting

- **Python** – automation, exploits, tooling  
- **Bash** – recon & workflow scripting  
- **JavaScript** – client-side security testing  
- **PowerShell** – Windows & AD attacks  
- **SQL** – injection & database exploitation  

---

## 🖥️ Operating Systems

- Linux (Kali, Parrot, Ubuntu)  
- Windows & Active Directory Environments  

---

## 🛠️ Security Tools

**Daily Drivers**
- Burp Suite
- Nmap
- Metasploit
- Wireshark

**Offensive Utilities**
- OWASP ZAP
- SQLmap
- Hydra
- Shodan
- Git

---

## 📌 Focus Areas (2025)

- Advanced API exploitation  
- Auth & Business Logic Flaws  
- Internal Network Attacks  
- Custom Recon & Automation Tools  

---

## 📫 Contact

- 🌐 https://balajisangana.pages.dev  
- 📧 balajisangana731@gmail.com  
- 💼 LinkedIn: /in/balaji-sangana  

---

> *“Break systems responsibly to make them stronger.”*
